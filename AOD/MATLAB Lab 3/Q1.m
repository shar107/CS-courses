...
Solve the initial value problem and plot the graph of solution  y  for  0 ≤x ≤2.
y" + 4 y' + 20 y = 23 sin⁡x - 15 cos⁡x,		y(0)=0,		y'(0)= -1
...
Enter the coefficients [a,b,c]: [1 4 20]
Enter the RHS function f(x): 23*sin(x)-15*cos(x)
If the problem has initial conditions then enter 1 else enter 2: 1
Enter the initial conditions [x0, y(x0), Dy(x0)]:[0 0 -1]
...
cos(4*x)*exp(-2*x) - 2^(1/2)*cos(x + pi/4)
